...
class UserProfileController extends Controller
{
    ...
    public function user_update(Request $request)
    {
        ...
        $request->validate([
            ...
            [
                'kelamin' => 'Mohon pilih jenis kelamin',
                'asal' => 'Mohon pilih kota asal',
                'lahir' => 'Mohon tanggal lahir diisi',
            ]);
        ...
        if ($request->pekerjaan == 'mahasiswa') {
            if ($request->univ == '') {
                Toastr::error('Mohon di isi tempat anda kuliah');

                return back();
            }
            $univ = $request->univ;
        }
        if ($request->pekerjaan == 'karyawan') {
            if ($request->office == '') {
                Toastr::error('Mohon di isi tempat anda bekerja');

                return back();
            }
            $tempat = $request->office;
        }
        if ($request->pekerjaan == 'lainnya') {
            if ($request->other == '') {
                Toastr::error('Mohon di isi deskripsi kegiatan anda');
        ...
        $userDetails = [
            ...
            'hp_darurat' => (int) $request->darurat,
            'pekerjaan' => $request->pekerjaan,
            'tempat_kerja' => $tempat,
            'kampus' => $univ,
            'password' => strlen($request->new_password) > 6 ? bcrypt($request->new_password) : auth('customer')->user()->password,
        ];
        if (auth('customer')->check()) {
