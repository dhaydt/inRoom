...
@section('title',\App\CPU\translate('Lanjutan Pembayaran'))
    ...
    <div class="row">
        <section class="col-lg-8">
            ...
            <div class="checkout_details mt-3">
                ...
                <h2 class="pb-5 mb-3 mt-4 payment-title">{{\App\CPU\translate('Lanjutan_Pembayaran')}}</h2>
                <div class="form-list">
                    <span class="subtitle mb-1 d-block">No. Booking</span>
                    <span class="content">{{ $room->order_id }}</span>
                    <hr class="line mt-4 mb-4">
                </div>
                <div class="form-list">
                    <span class="subtitle mb-1 d-block capitalize">Jenis pembayaran</span>
                    <span class="content">Pembayaran lanjutan</span>
                    <hr class="line mt-4 mb-4">
                </div>
                <div class="form-list">
                ...
                <div class="form-list d-flex justify-content-between w-100">
                    ...
                    <input type="hidden" name="value" value="{{ $pay}}" id="price">
                    <span class="content price">{{ \App\CPU\Helpers::currency_converter($pay)
                        }}</span>
                    <hr class="line mt-4 mb-4">
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        ...
                        <form class="needs-validation" target="_blank" method="POST" id="send"
                            action="{{route('xendit-payment.vaNextInvoice')}}">
                            <input type="hidden" id="type" name="type" value="">
                            <input type="hidden" name="order_id" value="{{ $room->id }}">
        ...
        <!-- Sidebar-->
        @php($detail = json_decode($room->order->details[0]->product_details))
        @php($sewa = json_decode($room->order->details[0]->data_penyewa))
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-8">
                            ...
                            <div class="mt-2">
                                <span class="price">
                                    Pembayaran bulan ke - {{$room->bulan_ke}}
                                </span>
                            </div>
                        </div>
                        @php($rooms = json_decode($room->order->details[0]->product_details))
                        <div class="col-4 img-div">
                            <img class="w-100" src="{{ asset('storage/product').'/'.json_decode($rooms->images)[0] }}"
                                style="border-radius: 6px">
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    @php($date = Carbon\Carbon::parse($room->deadline)->isoFormat('dddd, D MMMM Y'))
                    <div class="row px-0">
                        <div class="col-12 d-flex justify-content-between mt-2 px-0 mulai ">
                            <span class="capitalize title">Jatuh tempo pembayaran</span>
                        ...
                        <div class="col-12 d-flex justify-content-between mt-3 px-0 mulai">
                            ...
                            <span class="content">{{ $room->total_durasi }} Bulan</span>
                        </div>
                    </div>
                </div>
                {{-- {{ dd($order) }} --}}
...
<script type="text/javascript">
        ...
        function payNow(){
            ...
            $.ajax({
                url: "{{route('xendit-payment.vaNextInvoice')}}",
                method: 'POST',
                data: {
                    "id": id,
                    'type': payment,
